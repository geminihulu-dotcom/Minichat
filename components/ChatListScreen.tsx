...
const ChatListItem: React.FC<{ chat: ChatWithParticipant; onSelectChat: (chatId: string) => void }> = ({ chat, onSelectChat }) => {
    ...
    >
      ...
      <div className="flex-grow">
        <div className="flex justify-between items-center">
          ...
          <p className="text-xs text-gray-500">{formatTimestamp(lastMessage?.createdAt)}</p>
        </div>
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(chatsQuery, async (querySnapshot) => {
      ...
      if(userIds.length > 0) {
        ...
        const chatsWithParticipants = chatsData.map(chat => {
